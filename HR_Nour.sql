...
AS
    ...
    IF (EXISTS (
            ...
            WHERE E.Leave_ID = @request_ID
                AND E.status = 'pending' and E.emp1_ID<>@HR_ID
        ))
    ...
    BEGIN
        ...
        ELSE
            SET @balancecheck = 1;
    --checking if employee replaces anyone else
        
        SET @avail2check=dbo.checkingifreplacingsomeoneelse(@HR_id,@startdate,@enddate)
        ...
        BEGIN
            ...
            Update Employee_Approve_Leave set status='approved' where Emp1_ID=@HR_ID and leave_id=@request_id
        ...
        BEGIN
            ...
            Update Employee_Approve_Leave set status='rejected' where Emp1_ID=@HR_ID and leave_id=@request_id
        END
    END
...
BEGIN
    ...
    RETURN; -- If any previous pending just return who isnt hr...
    ...
    SELECT @replacementemp = replacement_emp
        ...
        ELSE
            SET @availcheck = 0
--checking if employee replaces anyone in that time period
 set @availcheck2=dbo.checkingifsomeoneelseelse(@replacementemp,@startdate,@enddate)

    -- Final check
    IF (@availcheck = 1 AND @spentcheck = 1 AND @reasoncheck = 1 and @availcheck2=1)
    BEGIN
        UPDATE Leave
        SET final_approval_status = 'approved'
        WHERE request_ID = @request_ID
...
if exists (select * from Employee_Replace_Employee E where E.emp2_ID=@employee_id and (E.from_date BETWEEN @startdate AND @enddate)
